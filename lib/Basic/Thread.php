...
class IMP_Basic_Thread extends IMP_Basic_Base
{
    ...
    protected function _init()
    {
        ...
        foreach ($imp_indices as $ob) {
            ...
            $fetch_res = $imp_imap->fetch($ob->mbox, $query, [
                ...
                switch ($mode) {
                    case 'thread':
                        ...
                        $curr_msg['link'] = $multiple
                            ? Horde::widget(['url' => '#display', 'title' => _('Thread List'), 'nocheck' => true])
                            : '';
                        $curr_tree['subject'] = $imp_mailbox->getThreadOb($imp_mailbox->getArrayIndex($fetch_res[$idx]->getUid(), $ob->mbox) + 1)->img;
                        break;

                    default:
                        $curr_msg['link'] = Horde::widget(['url' => '#display', 'title' => _('Back to Multiple Message View Index'), 'nocheck' => true]);
                        $curr_tree['subject'] = '';
                        break;
                }
        ...
        $imp_indices->flag([Horde_Imap_Client::FLAG_SEEN]);
     ...
     */
    protected function _buildAddressLinks(
        Horde_Mail_Rfc822_List $addrlist,
        $addURL = null,
        $link = true
    ) {
        ...
        foreach ($addrlist->base_addresses as $ob) {
            if ($ob instanceof Horde_Mail_Rfc822_Group) {
                ...
                foreach ($ob->addresses as $ad) {
                    ...
                    if ($link) {
                        $clink = new IMP_Compose_Link(['to' => strval($ad)]);
                ...
                if ($link) {
                    $clink = new IMP_Compose_Link(['to' => strval($ob)]);
