    ...
    protected function _init()
    {
        ...
        if (isset($this->vars->mailbox_list)) {
            if (is_array($this->vars->mailbox_list)) {
                ...
                $search_mailbox = [$default_mailbox];
            }
        ...
        if ($this->vars->criteria_form) {
                    ...
                    case 'flag':
                        ...
                        */
                        ...
                        if (strpos($id, '0\\') !== false) {
                            $flag =  substr($id, 2);
                            $formdata = ['flag' => $flag, 'set' => false];
                        } elseif (strpos($id, '%5C') !== false) {
                            $flag = strtoupper(substr($id, 3));
                            $formdata = ['flag' => $flag, 'set' => true];
                        } else {
                            $formdata = ['flag' => $id, 'set' => true];
                        }
            ...
            switch ($this->vars->search_type) {
                case 'filter':
                    $q_ob = $imp_search->createQuery($c_list, [
        ...
        if ($this->vars->edit_query && $default_mailbox->search) {
            ...
            foreach ($iterator as $val) {
                $rs[$val->formid] = [
                    'c' => $val->criteria,
                    'f' => [
                        'm' => IMP_Mailbox::formTo($val->all ? [IMP_Search_Query::ALLSEARCH] : array_map('strval', $val->mbox_list)),
            ...
            $js_vars['ImpSearch.i_mboxes'] = [
                'm' => $this->vars->subfolder ? [] : $s_mboxes,
                's' => $this->vars->subfolder ? $s_mboxes : [],
            ];
        }
        ...
        if ($default_mailbox->search) {
            ...
            $js_vars['ImpSearch.i_mboxes'] = [
                'm' => IMP_Mailbox::formTo($q_ob->all ? [IMP_Search_Query::ALLSEARCH] : $q_ob->mbox_list),
        ...
        $page_output->addInlineJsVars(array_merge($js_vars, [
