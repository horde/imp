...
class IMP_Ajax_Imple_ItipRequest extends Horde_Core_Ajax_Imple
{
            ...
            switch ($action) {
                case 'delete':
                    ...
                    if ($registry->hasMethod('calendar/delete')) {
                        ...
                        try {
                            $registry->call('calendar/delete', [$guid, $recurrenceId, $range]);
                    ...
                    switch ($components[$key]->getType()) {
                        ...
                        case 'vFreebusy':
                            ...
                            if ($registry->hasMethod('calendar/import_vfreebusy')) {
                                try {
                                    $registry->call('calendar/import_vfreebusy', [$components[$key]]);
                        ...
                        case 'vTodo':
                            ...
                            if ($registry->hasMethod('tasks/import')) {
                                try {
                                    $guid = $registry->call('tasks/import', [
                                        $components[$key],
                                        $mime_part->getType(),
                                    ]);
    ...
    protected function _handlevEvent($key, array $components, $mime_part)
    {
        ...
        try {
            ...
            $registry->call('calendar/export', [$guid, 'text/calendar', [], $calendars]);
            $success = true;
        } catch (Horde_Exception $e) {
            $success = false;
        }
        ...
        if ($registry->hasMethod('calendar/import')) {
            ...
            try {
                $guid = $registry->call('calendar/import', [
                    $components[$key],
                    $mime_part->getType(),
                ]);
                $url = Horde::url($registry->link('calendar/show', ['uid' => $guid]));
