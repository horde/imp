...
class IMP_Sentmail_Sql extends IMP_Sentmail
{
    ...
    public function favouriteRecipients($limit, $filter = null)
    {
        ...
        $where = '';
        if (!empty($filter)) {
            $filter = array_map([$this->_db, 'quote'], $filter);
            $where = sprintf(
                ' AND sentmail_action in (%s)',
                implode(', ', $filter)
            );
        }

        $query = sprintf(
            'SELECT sentmail_recipient, count(*) AS sentmail_count FROM %s WHERE sentmail_who = %s AND sentmail_success = 1%s GROUP BY sentmail_recipient ORDER BY sentmail_count DESC',
            $this->_params['table'],
    ...
    public function numberOfRecipients($hours, $user = false)
    {
        ...
        /* Execute the query. */
        try {
            return $this->_db->selectValue($query, [time() - $hours * 3600]);
        } catch (Horde_Db_Exception $e) {
            return 0;
        }
    }
    ...
    protected function _deleteOldEntries($before)
    {
        /* Build the SQL query. */
        $query = sprintf(
            'DELETE FROM %s WHERE sentmail_ts < ?',
            $this->_params['table']
        );

        /* Execute the query. */
        try {
            $this->_db->delete($query, [$before]);
        } catch (Horde_Db_Exception $e) {
        }
    }
